<?php
	...
	/**
	* This is the OCASADOMICILIO shipping module for Interspire Shopping Cart. To enable
	* OCASADOMICILIO in Interspire Shopping Cart login to the control panel and click the
	* Settings -> Shipping Settings tab in the menu.
	*/
	class SHIPPING_OCASADOMICILIO extends ISC_SHIPPING
	{

		/**
		* Variables for the OCASADOMICILIO shipping module
		*/

		/*
			The delivery type for OCASADOMICILIO shipments
		*/
		private $_deliverytype = "";

		/*
			The destination country ISO code for OCASADOMICILIO shipments
		*/
		private $_destcountry = "";

		/*
			The destination country zip for OCASADOMICILIO shipments
		*/
		private $_destzip = "";

		/*
			The shipping rate OCASADOMICILIO shipments
		*/
		private $_shippingrate = "";

		/*
			The packaging type for OCASADOMICILIO shipments
		*/
		private $_packagingtype = "";

		/*
			The destination type (residential or commercial) for OCASADOMICILIO shipments
		*/
		...
		*/
		...
		{
// Setup the required variables for the OCASADOMICILIO shipping module
			parent::__construct();
			$this->_name = GetLang('OCASADOMICILIOName');
			$this->_image = "ocasa_logo.gif";
			$this->_description = GetLang('OCASADOMICILIODesc');
			$this->_help = GetLang('OCASADOMICILIOHelp');
			$this->_height = 310;

			$this->_deliverytypes = array(
				"1DM" => GetLang('OCASADOMICILIODeliveryType1'),
				"1DA" => GetLang('OCASADOMICILIODeliveryType2'),
			);
		...
		*/
		...
		private function GetQuote()
		{
			...
			// Connect to OCASADOMICILIO.com to retrieve a live shipping quote
			$result = "";
			$valid_quote = false;
			$action = "3";
			$ups_url = "http://www.ocasa.com/using/services/rave/qcostcgi.cgi?accept_OCASADOMICILIO_license_agreement=yes&";

			// for some reason the options are stored url encoded (like with + instead of space)
			$shippingRate = urldecode($this->_shippingrate);

			// OCASADOMICILIO will only recognise the ZIP part of ZIP+4 for US addresses - drop it
			$zip = $this->_destzip;
			if ($this->_destcountry == 'US') {
				// replace either XXXXXYYYY or XXXXX-YYYY with just XXXX
				// shouldn't affect quotes since OCASADOMICILIO will throw an error for anything other than 5 digits
			...
			if($valid_quote) {
				...
				// Couldn't get to OCASADOMICILIO.com
				$this->SetError(GetLang('OCASADOMICILIOOpenError'));
				return false;
			}
		...
		public function GetServiceQuotes()
		{
			$this->ResetErrors();
			$QuoteList = array();
			// Set the OCASADOMICILIO-specific variables
			...
			// Return quotes for all available OCASADOMICILIO service types
		...
		/**
		* Generate a link to track items for OCASADOMICILIO.
		*
		* @return string The tracking URL for OCASADOMICILIO shipments.
		*/
