...
class ISC_ENTITY_CUSTOMER extends ISC_ENTITY_BASE
{
	...
	protected function getPosthook($customerId, &$customer)
	{
		...
		if (!isId($customer["custbrandid"])) {
			$query = "SELECT * FROM [|PREFIX|]brands";

			$result = $GLOBALS["ISC_CLASS_DB"]->Query($query);
			$row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result);

			if ($row) {
				$customer["custbrandid"] = $row["brandid"];
			}
		}

		$customer['customerbrand'] = null;
		if (isId($customer["custbrandid"])) {
			$customer["customerbrand"] = $this->group->get($customer["custbrandid"]);
		}		
	...
	protected function addPrehook(&$savedata, $rawInput)
	{
		...
		if (!array_key_exists("custbrandid", $rawInput) || !isId($rawInput["custbrandid"])) {
			$savedata["custbrandid"] = 0;
		}
	...
	/**
	 * Edit the customer's brand ID
	 *
	 * Method will only edit the customer's brand ID
	 *
	 * @access public
	 * @param int $customerId The customer ID
	 * @param int $customerGroupId The new customer brand ID. Default is 0 (the default brand)
	 * @return bool TRUE if the customer's brand was successfully updated, FALSE otherwise
	 */
	public function editBrand($customerId, $customerBrandId=0)
	{
		if (!isId($customerId) || trim($customerBrandId) == "") {
			return false;
		}

		$savedata = array(
			"custbrandid" => $customerBrandId
		);
