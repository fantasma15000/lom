...
*/
	...
	*/
	private function SaveShippingProvider()
	{
		...
		$sh = $GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerShippingAddresses();
		
		$this->SetOrderShippingAddress("dfsfds",true);
		
		//actualiza telefono de shipping address
		foreach($sh as $key => $value){
			$custid=$value["shipcustomerid"];
			break;
		}
		
		$updatedSATel = array(
			"shipphone" => $_POST['telparasucursal']
		);
		$telforsuc = $_POST['telparasucursal'];				
		$GLOBALS['ISC_CLASS_DB']->UpdateQuery("shipping_addresses", $updatedSATel, "shipcustomerid = ".$custid);
		...
		$nombreSucursal = $this->getSucursalName($_POST['ShippingSucursal']);
		$sucid = $_POST['ShippingSucursal'];
		...
		if($_POST['ShippingType'] == 1){
			$id = 12;
			$pre = $_POST['precioSucursalHidden'];
			$preText = "Entrega en Sucursal ".$nombreSucursal." - DNI: ".$_POST['dniparasucursal'] . " - TEL: ".$telforsuc;
		}elseif($_POST['ShippingType'] == 2){	
			$id = 13;
			$pre = $_POST['precioDomicilioHidden'];
			$preText = "En Domicilio:\n ".$_POST['domicilioaddress']." (".$_POST['domiciliostate']." | ".$_POST['domiciliocountry'].") - ".$_POST['domiciliophone'];
		}
		
		
		//$cachedShippingMethod = $shippingAddress->getCachedShippingMethod($id);
		//var_dump ($cachedShippingMethod);
	...
	/**
	 * get the sucursal name form db by id.
	 *
	 * @param id: sucursal db id
	 * @return string: sucursal name.
	 */	
	private function getSucursalName($id){
			$queryS = "SELECT sucursalname, sucursaladdress FROM isc_sucursales where sucursalid=".$id;
			$resultS = $GLOBALS['ISC_CLASS_DB']->Query($queryS);
			while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($resultS)) {	
				return $row['sucursalname'] . " (".$row['sucursaladdress'].") ";
			}
	...
	private function ChooseBillingAddress($errors=array())
	{
		...
		$GLOBALS['FromURL'] = urlencode("checkout.php?action=confirm_order");
...
/*			if ($this->GetNumShippingAddresses() == 0) {
	...
	*/
	...
	public function BuildOrderConfirmation()
	{
		...
		//$this->createShippingBox($GLOBALS['ItemId']);

		//Puede continuar?
		$GLOBALS['HideButtomPaymentButton2']="none";
		if(!$this->getQuote()->isDigital()) {  
			$GLOBALS['HideButtomPaymentButton']="none";
			$GLOBALS['HideButtomPaymentButton2']="inline-block";
			$items = $this->getQuote()->getItems();
			
			$shippingAddresses = $this->getQuote()->getShippingAddresses();
			foreach($shippingAddresses as $address) {
				if($address->hasShippingMethod()) {
					 $GLOBALS['HideButtomPaymentButton']="inline-block";
					 $GLOBALS['HideButtomPaymentButton2']="none";
				}
			}
		}
		...
		if(!$this->getQuote()->isDigital()) {  
			...
			$GLOBALS['HideShippingDetails'] = 'display:inline-block';
		 }
