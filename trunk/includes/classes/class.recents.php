<?php
	...
	class ISC_RECENTS
	{
	    ...
	    private $_recent_products_per_page =12;
		...
		{
			$this->_recent_products_per_page=12;
			$GLOBALS['CatId'] = 0;
			$this->_catlayoutfile = "recents";
		}
		
		public function GetProductsPerPage()
		{
		 return $this->_recent_products_per_page;
		}
		...
		public function SetNumPages()
		{
			...
			if ($this->_recent_products_per_page > 0) {
				$this->_catnumpages = ceil($this->GetNumProducts() / $this->_recent_products_per_page);
			}
			else {
				$this->_catnumpages = 0;
			}
		...
		public function SetStart()
		{
			...
			switch ($this->_catpage) {
				...
				// Page 2 or more
				default: {
					$start = ($this->GetPage() * $this->_recent_products_per_page) - $this->_recent_products_per_page;
					break;
				}
			}
		...
		public function SetNumProductshc($numproducts)
		{
		 $this->_catnumproducts=$numproducts;
		}
		...
		public function SetNumProducts()
		{
			...
			$query = "
				...
				FROM
					...
					".$taxJoin."
				WHERE
					".$this->getProductWhereSQLRestriction()." and  publish_end < now()
			";
		...
		public function SetCategoryData()
		{
			...
			//$aditionalWhere=" and (prodfeatured=1 and (publish_start < now() and publish_end > now() ))";
			$aditionalWhere="";
			
			$this->LoadProductsForPage($aditionalWhere);
		...
		public function ShowCategory()
		{
			...
			if($this->GetPage() != 1) {
				$canonicalLink = RecentLink( array('page' => $this->GetPage()));
			} else {
				$canonicalLink = RecentLink();
			}
		...
		public function LoadProductsForPage($aditionalWhere=null)
		{
			...
			$query = "
				SELECT
					p.*, b.brandname as brandName, b.brandinfo as brandInfo, b.brandgmap as brandMap, b.brandimagefile as BrandImageFile,
				...
				FROM
					(
						...
						FROM
							...
							".$taxJoin."
						WHERE 
							".$this->getProductWhereSQLRestriction(). $aditionalWhere . "
							
						ORDER BY
							" . $this->GetSortField() . ", p.prodname ASC
						" .	$GLOBALS['ISC_CLASS_DB']->AddLimit($this->GetStart(), $this->_recent_products_per_page) . "
					) AS ca
					INNER JOIN [|PREFIX|]products p ON p.productid = ca.productid
		...
		static public function buildSearchResultsHTML()
		{
			...
			foreach ($results["results"] as $category) {
				...
				foreach ($catBreadCrumbs as $part) {
					$link[] = "<a href=\"" . Link($part[0], $part[1], $isParent) . "\">" . isc_html_escape($part[1]) . "</a>";
