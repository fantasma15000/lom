<?php
	...
	class ISC_PRODUCT
	{
		...
		public $_prodname = "";
		public $_prodpublishend = "";
		public $_prodpublishstart = "";
		public $_prodthumb = "";
		public $_proddesc = "";
		public $_prodbrandname = "";
		public $_prodbrandinfo = "";
		public $_prodbrandlink = "";
		public $_prodbrandmap = "";
		public $_prodbrandimagefile = "";
		public $_prodbrandimage = "";
		...
		public function _SetProductData($productid=0)
		{
			...
			$query = "
				...
				(SELECT brandinfo FROM [|PREFIX|]brands WHERE brandid=p.prodbrandid) AS prodbrandinfo,
				(SELECT brandgmap FROM [|PREFIX|]brands WHERE brandid=p.prodbrandid) AS prodbrandmap,
				(SELECT brandlink FROM [|PREFIX|]brands WHERE brandid=p.prodbrandid) AS prodbrandlink,
			...
			$this->_prodpublishstart = $row['publish_start'];
			$this->_prodpublishend = $row['publish_end'];
			...
			$this->_prodbrandinfo = $row['prodbrandinfo'];
			$this->_prodbrandlink = $row['prodbrandlink'];
			$this->_prodbrandmap = $row['prodbrandmap'];
		...
		*/
		public function GetCombinationDetails($combination, $customerGroupId = null)
		{
			...
			$GLOBALS['RetailPrice'] =  sprintf("%01.2f",$row['prodretailprice']);
			$GLOBALS['OurPrice'] = sprintf("%01.2f",$row['prodprice']);
			$GLOBALS['YouSave'] = sprintf("%01.2f",$row['prodretailprice'] - $row['prodprice'])  ;			
			$percent = $calculatedPrice / $rrp*100;
			$GLOBALS['Percent'] =  sprintf("%01.2f", $percent ); 
			...
			foreach ($crumbList as $productcatid => $trail) {
				...
				foreach ($cats as $categoryid) {
					...
					if (isset($categories[$categoryid])) {
						...
						$GLOBALS['ThisProductCategory'] = $catname;
		...
		public function GetProductPublish_Start()
		{
			return $this->_prodpublishstart;
		}		
		
		public function GetProductPublish_End()
		{
			return $this->_prodpublishend;
		}		
		...
		{
			return $this->_prodbrandinfo;
		}

		public function GetBrandLink()
		{
			return $this->_prodbrandlink;
		}		
		
		public function GetBrandMap()
		{
			return $this->_prodbrandmap;
		}		
