...
require_once(ISC_BASE_PATH . '/lib/addressvalidation.php');
	...
	class ISC_ACCOUNT
	{
		...
		*/
		private function SaveEditedShippingAddress()
		{
			if (isset($_POST['shipid'])) {
				...
				$_SESSION['LOGIN_REDIR'] = sprintf("%s/checkout.php?action=confirm_order", $GLOBALS['ShopPath']);
		...
		*/
		...
		public function MyAccountPage()
		{
		  $customer = $GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerDataByToken();
		    ...
		    $GLOBALS['Credit'] = number_format ($customer['custstorecredit'],2);
			...
			$GLOBALS['ISC_LANG']['AccountValidarCupones'] = sprintf(GetLang('AccountValidarCupones'), $GLOBALS['StoreName']);
			$GLOBALS['ISC_LANG']['AccountValidarCuponesDesc'] = sprintf(GetLang('AccountValidarCuponesDesc'), $GLOBALS['StoreName']);


					if($customer['custbrandid'] != 0 && $customer['custbrandid'] != ""){
						$GLOBALS['ShopValidarCuponesOption'] = "<li><a href='".$GLOBALS['ShopPath']."/account.php?action=validarcupones'>".$GLOBALS['ISC_LANG']['AccountValidarCupones']."</a> - ".$GLOBALS['ISC_LANG']['AccountValidarCuponesDesc']."</li>";
						$GLOBALS['ShopValidarCuponesOption'] .= "<li><a href='".$GLOBALS['ShopPath']."/account.php?action=misofertas'>".$GLOBALS['ISC_LANG']['AccountMisOfertas']."</a> - ".$GLOBALS['ISC_LANG']['AccountMisOfertasDesc']."</li>";
					}else{
						$GLOBALS['ShopValidarCuponesOption'] = "";
					}
		...
		*/
		public function GetCustomerCupones(&$Result, $status=null,$fecha=null)
		{
			$complete_filter = "";

			if ($status) {
				$complete_filter .= " and (compraStatusId='$status' ) ";
			}
			if ($fecha) {
				if ($fecha==1)
					$complete_filter .= " and (Validez>=now() ) ";
				else
				    $complete_filter .= " and (Validez<now() ) ";
			}
			...
			$query = "
				select c.*, s.statusid, s.statusdesc,p.* , sh.cost_inc_tax as sh_price, sh.method as sh_method, c.compraid as compraid
from isc_compra c
inner join isc_compra_status s  on c.compraStatusid=s.statusid
inner join isc_products p on p.productid=c.productid
inner join isc_orders O on c.orderid=O.orderid
left join isc_order_shipping as sh on O.orderid = sh.order_id
WHERE O.ordcustid='" . $GLOBALS['ISC_CLASS_DB']->Quote($GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerId()) . "'
            AND O.deleted = 0 " . $complete_filter . "
...
FROM isc_order_products p INNER JOIN isc_orders o on p.orderorderid=o.orderid
        ...
        INNER JOIN isc_compra com on com.orderId=o.orderid  where md5(com.compraid)='" . $md5id. "'";
				...
				$shipdata = "";
					while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {	
						...
						$this->PrintCuponForCustomer($row['token'],$row['control'],$row['custconfirstname'],$row['custconlastname'],$row['custconemail'],$row['validez'],$row['ProductMailDescription'],$row['brandname'],$row['brandinfo'], $shipdata );
					}
		...
		public function PrintCuponForCustomer($cuponToken,$control,$first,$last,$email,$validez,$maildescription,$brandname,$brandinfo, $shipdata = "")
		{
		   $GLOBALS['CuponToken'] = $cuponToken;
		   $GLOBALS['Control'] = $control;
		   $GLOBALS['First'] = $first;
		   ...
		   $emailTemplate = FetchEmailTemplateParser();
			...
			echo "<form><input type='button' value='Imprimir' onclick='window.print();return false;' /><input type='button' value='Cerrar' onclick='window.close();return false;' /></form><br clear='all' />";
			echo $message;
			die();		
	}	
...
FROM isc_order_products p INNER JOIN isc_orders o on p.orderorderid=o.orderid
		...
		/**
		* GASTON: validar cupones
		*/
		
		private function Validarcupones()
		{
				$GLOBALS['ValidarCupones_ofertaID'] = "";
				$customer = $GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerDataByToken();
				$brandid = $customer['custbrandid'];
				
			
				
				
				if(isset($_REQUEST['validarcuponessent']) && $_REQUEST['validarcuponessent'] == 1){				
					$query = "SELECT c.compraid as cid, p.productid as pid, p.prodname as prodname, cs.statusdesc as statusdesc, i.imagefiletiny as imagefiletiny, 
										c.compraStatusid as compraStatusid, cus.custconfirstname as fn, cus.custconlastname as ln
								FROM isc_compra AS c
								INNER JOIN isc_products AS p ON c.productid = p.productid
								INNER JOIN isc_compra_status AS cs ON cs.statusid = c.compraStatusid
								INNER JOIN isc_orders AS o ON o.orderid = c.orderid
								INNER JOIN isc_customers AS cus ON cus.customerid = o.ordcustid
								LEFT JOIN isc_product_images AS i ON i.imageprodid = p.productid
								WHERE p.prodbrandid = ".$brandid." AND (c.compratoken = '".$_REQUEST['cod1']."' AND c.compraSeguridad = '".$_REQUEST['cod2']."')
								GROUP BY c.compraid";
								
					$resultQuery = $GLOBALS['ISC_CLASS_DB']->Query($query);
					while ($rows = $GLOBALS['ISC_CLASS_DB']->Fetch($resultQuery)) {				  
						$GLOBALS['ValidarCupones_ofertaID'] = $rows['pid'];
						$GLOBALS['ValidarCupones_oferta'] = $rows['prodname'];
						$GLOBALS['ValidarCupones_estado'] = $rows['statusdesc'];
						$GLOBALS['ValidarCupones_user'] = $rows['fn']." ".$rows['ln'];
						$GLOBALS['Thumbimage'] = $rows['imagefiletiny'];
						
						$valcid = base64_encode($rows['cid']);
						
						if($rows['compraStatusid'] == 10){
						     $GLOBALS['Mensaje'] = "<span style=' font-size:20px; color:green;'>  CUPON VALIDO </span>";
							 
									$ccid = base64_decode($_REQUEST['val']);
									$updateQuery = "UPDATE isc_compra SET compraStatusid = 2   WHERE compratoken = '".$_REQUEST['cod1']."' AND compraSeguridad = '".$_REQUEST['cod2']."'";
									$GLOBALS['ISC_CLASS_DB']->Query($updateQuery);
								
								
										
						} else {
						 $GLOBALS['Mensaje'] = "<span style=' font-size:20px; color:red;'>   ESTE CUPON YA FUE UTILIZADO </span>";
							$GLOBALS['ValidarCupones_ofertaBoton'] = "";
						}
					}
					if($GLOBALS['ValidarCupones_ofertaID'] != ""){
						$GLOBALS['HideCuponesVList'] = "block";
						$GLOBALS['HideCuponesVListEmpty'] = "none";
					}else{
						$GLOBALS['HideCuponesVList'] = "none";
						$GLOBALS['HideCuponesVListEmpty'] = "block";				
					}
					
					$GLOBALS['SNIPPETS']['AccountCuponesValidados'] = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("AccountCuponesValidados");
				} else {
					$GLOBALS['HideCuponesVList'] = "none";
					$GLOBALS['HideCuponesVListEmpty'] = "none";
					$GLOBALS['SNIPPETS']['AccountCuponesValidados'] = "";
				}
			
			
			
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle(sprintf("%s - %s", GetConfig('StoreName'), "Cupones"));
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("account_validarcupones");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}		
		...
		*/
		...
		private function Misofertas()
		{
				$GLOBALS['SNIPPETS']['AccountMisOfertas'] = "";
				$GLOBALS['MisOfertas_oferta'] = "";
				$customer = $GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerDataByToken();
				$brandid = $customer['custbrandid'];					
					
					$query = "
					select p.productid, prodname, count(distinct u.compraid) as utilizados,  count(distinct d.compraid) as disponibles, i.imagefiletiny as imagefiletiny from isc_products p left join isc_compra u on u.productid=p.productid and u.compraStatusid=2 left join isc_compra d on d.productid=p.productid and d.compraStatusid=10 

LEFT JOIN isc_product_images i ON i.imageprodid = p.productid 

where p.prodbrandid = ".$brandid." 
...
";
					...
					$resultQuery = $GLOBALS['ISC_CLASS_DB']->Query($query);
					while ($rows = $GLOBALS['ISC_CLASS_DB']->Fetch($resultQuery)) {	
						$GLOBALS['MisOfertas_oferta'] = $rows['prodname'];
						$GLOBALS['ThumbimageOf'] = $rows['imagefiletiny'];					
						$GLOBALS['Disponibles'] = $rows['disponibles'];
						$GLOBALS['Utilizados'] = $rows['utilizados'];
						$GLOBALS['Total'] = ($rows['disponibles'] + $rows['utilizados']);
						$GLOBALS['SNIPPETS']['AccountMisOfertas'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("AccountMisOfertas");
					}
					
					if($GLOBALS['MisOfertas_oferta'] != ""){
						$GLOBALS['HideOfertasVList'] = "block";
						$GLOBALS['HideOfertasVListEmpty'] = "none";
					}else{
						$GLOBALS['HideOfertasVList'] = "none";
						$GLOBALS['HideOfertasVListEmpty'] = "block";			
					}
					
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle(sprintf("%s - %s", GetConfig('StoreName'), "Ofertas"));
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("account_misofertas");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}		
		...
		/**
		*	JULIAN_: Show a list of Cupones		
		*/
		private function Cupones()
		{

		//$GLOBALS['ISC_CLASS_DB']->Query('UPDATE isc_compra SET compraStatusid = 9 where orderid="10000008"');
		//$dd = mysql_query('UPDATE isc_compra SET compraStatusid = 10 where orderid=10000023');
		//echo $dd;die('UPDATE isc_compra SET compraStatusid = 9 where orderid="10000008"');
		
			$GLOBALS['SNIPPETS']['AccountCuponStatus'] = "";
			$result = false;
			$status=null;
			$fecha=null;
			if (isset($_REQUEST['a'])) {
			...
			$this->GetCustomerCupones($result,$status,$fecha);

			while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				$order = $row;
				
				$GLOBALS['OrderId'] = $row['orderid'];
				$GLOBALS['CuponName'] = $row['prodname'];
				$GLOBALS['ShippingInfoData'] = "";
				$GLOBALS['Validez'] =  substr($row['compraValidez'],0,10);	
				$GLOBALS['CuponStatus'] = $row['statusdesc'];		
				if ($row['statusid']==10)
					{$GLOBALS['CuponToken'] = "C&oacute;digo de Seguridad:".$row['compratoken'] . "<br/> C&oacute;digo de Control:" . $row['compraSeguridad'];  ;	
					 $GLOBALS['Reenviar'] = "<br/><a href=" .$GLOBALS['ShopPath'] . "/account.php?action=reenviar&id=" . md5($row['compraid'])  . ">Reenviar</a>";
					 $GLOBALS['Imprimir'] = "<a target='_blank' href=" .$GLOBALS['ShopPath'] . "/account.php?action=imprimir&id=" . md5($row['compraid'])  . ">Imprimir</a>";
					
					/* agregar info de envio */
					if($row['sh_method'] != ""){ 
					 $GLOBALS['ShippingInfoData'] = "M&eacute;todo de Env&iacute;o: ".$row['sh_method']."<br /><br />";
					}else{$GLOBALS['ShippingInfoData'] = "";}
					} 
					else
					{
					$GLOBALS['CuponToken']="";
				...
				$query = "SELECT imagefiletiny FROM isc_product_images i where imageprodid=" .$row['productid'] . " limit 1";
				$resultimage = $GLOBALS['ISC_CLASS_DB']->Query($query);
				while ($rowImage = $GLOBALS['ISC_CLASS_DB']->Fetch($resultimage)) {
				  $GLOBALS['Thumbimage'] = GetConfig('AppPath').'/'.getConfig('ImageDirectory').'/'. $rowImage['imagefiletiny'];	
				...
				$GLOBALS['SNIPPETS']['AccountCuponStatus'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("AccountCuponStatusItem");
			}

			if($GLOBALS['SNIPPETS']['AccountCuponStatus']) {
				$GLOBALS['HideNoOrderStatusMessage'] = "none";
			}
			else {
				$GLOBALS['HideOrderStatusList'] = "none";
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle(sprintf("%s - %s", GetConfig('StoreName'), "Cupones"));
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("account_cuponstatus");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}
		...
		*/
		...
		private function ViewOrders()
		{
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle(sprintf("%s - %s", GetConfig('StoreName'), GetLang('YourCupons')));
