...
CLASS ISC_PRODUCTDETAILS_PANEL extends PANEL
{
	...
	public function SetPanelSettings()
	{
		...
		$startDate = isc_html_escape($this->productClass->GetProductPublish_Start());	
		$oStartDate= new DateTime( $startDate);
		...
		$sd_1 = explode("-", $sd[0]);
		$sd_2 = explode(":", $sd[1]);
		$dias = $sd_1[0].",".$sd_1[2].",".$sd_1[1];
		$horas = $sd_2[0].",".$sd_2[1].",".$sd_2[2];
		$startDate = $dias.",".$horas;
		$GLOBALS['ProductPublishStart'] = $startDate;
		$endDate = isc_html_escape($this->productClass->GetProductPublish_end());
		
		

		//$res = $this->compara_fechas( $actual,$endDatee); 		
		
		$oActual= new DateTime( $actual);
		$oEndDate= new DateTime( $endDate);
		...
		$endDate = str_replace("ART", "", $endDate);
		$endDate = str_replace("+0000", "", $endDate);
		$endDate = str_replace("GMT", "", $endDate);
		$endDate = str_replace("-0300", "", $endDate);


		if ($oActual<$oEndDate && $oActual>$oStartDate ) {
			...
			$GLOBALS['botonCompra'] = '<div style="float:right;"><input class= "addtcart" type="image" src="%%GLOBAL_IMG_PATH%%/red/compra.jpg" alt="" /></div>';
			
		$showActualJS = date("D M j Y 00:00:00", time()+86400);
		$GLOBALS['ProductPublishEnd'] = $showActualJS;			
			
		}else{	
			$GLOBALS['MensajeOferta']= '<img border="0" src="%%GLOBAL_IMG_PATH%%/IcoError.gif">La Oferta No est&aacute; activa';
			$GLOBALS['botonCompra'] = '<div style="float:right;"><img src="%%GLOBAL_IMG_PATH%%/red/agotado.jpg" alt="Agotado" /></div>';
		...
		//Muestro en el contador como fecha de cierre mañana a las 0hs
		
					/*$showActual = date("Y-m-d 00:00:00", time()+86400);
					$res1 = $this->compara_fechasc($endDatec, $showActual); 
					if($res1 > 0){					
						//si la fecha de cierre real es mayor a mañana a las 0 muestro que termina hoy a las 0
						$showActualJS = date("D M j Y 00:00:00", time()+86400);
						$GLOBALS['ProductPublishEnd'] = $showActualJS;
					} else {
						//si la fecha de cierre real es menor a mañana a las 0 muestro que termina en esa fecha
						$GLOBALS['ProductPublishEnd'] = $endDate;
					}*/			
		...
		$comprados = "SELECT sum(ordprodqty) FROM isc_order_products WHERE ordprodid = ".$this->productClass->GetProductId();
	...
	*/
		...
		if($retailPrice) {
			...
			if($productPrice < $retailPrice) {
				...
				if($this->productClass->GetProductCallForPricingLabel()) {
					...
					$string = sprintf(getLang('YouSave'), formatPrice($savings));
					$GLOBALS['YouSave'] = $string;
					$percent =  $savings *100/ $retailPrice;		
	...
	public function SetBrandDetails()
	{
		...
		$GLOBALS['BrandInfo'] = nl2br($this->productClass->GetBrandInfo());
		$GLOBALS['BrandMap'] = $this->productClass->GetBrandMap();
		$GLOBALS['BrandImageFile'] = isc_html_escape($this->productClass->GetBrandImageFile());
		$GLOBALS['BrandLink'] =  $this->productClass->GetBrandLink();
	}
