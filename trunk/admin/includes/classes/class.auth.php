<?php
	...
	class ISC_ADMIN_AUTH
	{
		...
		public function HandleSTSToDo($ToDo)
		{
			...
			else if(is_numeric(isc_strpos($do, "sucursal"))) {
				$GLOBALS['ISC_CLASS_ADMIN_SUCURSALES'] = GetClass('ISC_ADMIN_SUCURSALES');
				$GLOBALS["ISC_CLASS_ADMIN_SUCURSALES"]->HandleToDo($ToDo);
			}			
			...
			else if(is_numeric(isc_strpos($do, "ocasa"))) {
				$GLOBALS['ISC_CLASS_ADMIN_EXPORTOCASA'] = GetClass('ISC_ADMIN_EXPORTOCASA');
				$GLOBALS['ISC_CLASS_ADMIN_EXPORTOCASA']->HandleToDo($ToDo);
			}
		}
	}
