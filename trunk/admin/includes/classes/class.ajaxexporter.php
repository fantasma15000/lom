...
abstract class ISC_ADMIN_AJAXEXPORTER extends ISC_ADMIN_BASE
{
	...
	public function AutoExport()
	{
		...
		if(!empty($_GET['action']) && ($_GET['action'] == 'DescuentoCity' || $_GET['action'] == 'pinggers' || $_GET['action'] == 'dealandia')) {
			$exportResult = $this->Export(true);
			$file = APP_ROOT."/../cache/".$exportResult;
			Interspire_Download::downloadFile($file, $exportResult, true, 'text/xml', GetConfig('CharacterSet'));
		}else{
	...
	abstract public function WriteRow($row);
        ...
        abstract public function WriteDealandiaRow($row, $categ);

	protected function WriteRows($result)
	{


                if ($_GET['action'] == 'dealandia'){
                    $wroteRows = false;
                    $categ = '';
                    if (!empty($result)) {
                            while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
                                    $wroteRows = true;
                                    $this->WriteDealandiaRow($row, $categ);
                                    $categ = $row['catname'];
                            }
                    }
                    return $wroteRows;
