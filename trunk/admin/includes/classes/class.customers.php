<?php
	...
	//				);
		...
		*/
		...
		public function AddCustomerStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			if($PreservePost == true) {
				...
				$customerBrandId = $customer['custbrandid'];
				...
				$GLOBALS['CustomerBrandId'] = $customer['custbrandid'];
			...
			$GLOBALS['CustomerBrandOptions'] = $this->GetCustomerBrandsAsOptions($customerBrandId);
		...
		public function EditCustomerStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			$GLOBALS['CustomerBrandOptions'] = $this->GetCustomerBrandsAsOptions($customer['custbrandid']);
			$GLOBALS['CustomerShippingAddressGrid'] = $this->ManageCustomerAddressGrid();
		...
		private function _GetCustomerData($CustomerId=null, $entitiesData=true)
		{
			...
			if (!isId($CustomerId)) {
				...
				$customer['custbrandid'] = @$_POST['custbrandid'];
		...
		/**
		* _GetBrandList
		* Get a list of brands and return them as an array
		*
		* @param Int $NumBrands A reference variable to hold the number of groups
		* @return Array The list of brands
		*/
		private function _GetBrandList(&$NumBrands)
		{
			$brands = array();
			$query = "SELECT * FROM [|PREFIX|]brands ORDER BY brandname ASC";
			$resultB = $GLOBALS["ISC_CLASS_DB"]->Query($query);
			$NumBrands = $GLOBALS["ISC_CLASS_DB"]->CountResult($resultB);

			while($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($resultB)) {
		...
		*/
		...
		/**
		* GetCustomerBrandsAsOptions
		* Return a list of option tags containing name/values of customer groups
		*
		* @param Int $SelectedBrand The group to mark as selected in the option tags
		*
		* @return String The HTML <option> tags
		*/
		public function GetCustomerBrandsAsOptions($SelectedBrand=0)
		{
			$this->_customerBrands = &$this->_GetBrandList($numBrands);
			$options = "";

			foreach($this->_customerBrands as $brand) {
				...
				$options .= sprintf("<option value='%d' %s>%s</option>", (int) $brand['brandid'], $sel, isc_html_escape($brand['brandname']));
			}
