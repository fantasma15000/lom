	...
	class ISC_ADMIN_PRODUCT extends ISC_ADMIN_BASE
	{
		...
		*/
		...
		public function _GetProductData($ProductId = 0, &$RefArray = array())
		{
			...
			if ($ProductId == 0) {
				...
				$RefArray['prodocasaname'] = $_POST['prodocasaname'];
				
				$RefArray['publish_start'] = $_POST['prodPDate_from'];
				$RefArray['publish_end'] = $_POST['prodPDate_to'];
				
				$RefArray['ProductMailDescription'] = $_POST['ProductMailDescription'];
				$RefArray['Validez'] = $_POST['Validez'];
				...
				$RefArray['prodfixedshippingcost2'] = DefaultPriceFormat($_POST['prodFixedCost2']);
				$RefArray['shippingallowed'] = DefaultPriceFormat($_POST['ShippingAllowed']);
		...
		*/
		...
		public function AddProductStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			if($PreservePost == true) {
				...
				$GLOBALS['prodocasaname'] = isc_html_escape($arrData['prodocasaname']);
				...
				if (CFloat($arrData['prodfixedshippingcost2']) > 0) {
					$GLOBALS['ProdFixedShippingCost2'] = number_format($arrData['prodfixedshippingcost2'], GetConfig('DecimalPlaces'), GetConfig('DecimalToken'), "");
				}
				$GLOBALS['ShippingAllowed'] = $arrData['shippingallowed'];
		...
		*/
		...
		public function EditProductStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			if (ProductExists($prodId)) {
				...
				$GLOBALS['prodPDate_from'] = isc_html_escape($arrData['publish_start']);
				$GLOBALS['prodPDate_to'] = isc_html_escape($arrData['publish_end']);
				$GLOBALS['ProductMailDescription'] = isc_html_escape($arrData['ProductMailDescription']);
				$GLOBALS['Validez'] = isc_html_escape($arrData['Validez']);
				$GLOBALS['prodocasaname'] = isc_html_escape($arrData['prodocasaname']);
		...
		*/
		public function BulkEditProductsStep1($MsgDesc = "", $MsgStatus = "")
		{
			...
			$query = sprintf("
				SELECT productid, prodname, prodprice, prodfreeshipping, prodvisible, prodfeatured, prodvendorfeatured, prodbrandid, p.prodocasaname as prodocasaname
			...
			while($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
				...
				$GLOBALS['prodocasaname'] = isc_html_escape($row['prodocasaname']);
...
if($GLOBALS['ProdWeight'] == ""){$GLOBALS['ProdWeight'] = number_format(1, GetConfig('DecimalPlaces'), GetConfig('DecimalToken'), "");}
				if (CFloat($arrData['prodwidth']) > 0) {
